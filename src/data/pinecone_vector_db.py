...
class PineConeVectorDB:
    ...
    def index(self) -> pinecone.index.Index:
         ...
         if not self._index:
              self.log.error("Index has not been initialized. Please call `add_embeddings_to_pinecone_index` first.")
              raise ValueError("Index has not been initialized. Please call `add_embeddings_to_pinecone_index` first.")
         return self._index
    

    def add_embeddings_to_pinecone_index(self, embeddings: List[List[float]], dim: int = 384, metric: str = 'euclidean',cloud: str = 'aws',region: str = 'us-east-1') -> None:
       """
       ...
       if not self.index_name:
           self.log.error("Index name cannot be empty")
           raise ValueError("Index name cannot be empty")
       
       if self._index:
            self.log.error(f"Index with name {self.index_name} has been initialized. Cannot initialize again.")
       ...
       try:
           if self.index_name not in self._pineconedb.list_indexes().names:
                self.log.info(f"Creating Pinecone index: {self.index_name}")
                self._pineconedb.create_index(
                ...
                self.log.debug(f"Successfully created Pinecone index: {self.index_name}")
           else:
                self.log.info(f"Index {self.index_name} already exists, using existing index.")
            # Connect to the index
           self._index = self._pineconedb.Index(self.index_name)
           # Upload section embeddings
           for i, embedding in enumerate(embeddings):
                self._index.upsert([(str(i), embedding)])
           self.log.info(f"Successfully uploaded {len(embeddings)} embeddings to Pinecone index: {self.index_name}")

       except Exception as e:
            self.log.error(f"An error occurred when creating or upserting to Pinecone Index: {self.index_name}. Error: {e}")
            raise
    ...
    def get_vectordb_search_results(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        try:
            self.log.info(f"Searching Pinecone index: {self.index_name} for query: {query}")
            ...
            self.log.debug(f"Search results: {search_results}")
            return search_results
        except Exception as e:
           self.log.error(f"An error occurred while searching Pinecone index: {self.index_name} for query: {query}. Error: {e}")
           return {}
    ...
    def output_search_results_to_file(self, prefix: str, keyword: str, search_results: Dict[str, Any], sections: List[str]) -> None:
        ...
        try:
            ...
            if search_results and search_results.get("matches"):
                ...
                self.log.info(f"Saved search results to file: {full_path}")
            else:
                self.log.warning(f"No search results found for keyword: {keyword}")
        except FileNotFoundError as e:
             self.log.error(f"FileNotFoundError: {e} when saving file.")
        except KeyError as e:
             self.log.error(f"KeyError: {e} when extracting the data.")
        except Exception as e:
             self.log.error(f"An unexpected error occurred: {e} when saving the results. Error: {e}")